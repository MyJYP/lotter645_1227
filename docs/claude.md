...
로또 645 복권의 과거 당첨 데이터(601회~1205회, 2014.6.7~2026.1.3)를 Python으로 종합 분석하는 프로젝트입니다.

**작성일**: 2025-12-27
**최종 업데이트**: 2026-01-24
**버전**: v5.1.1
**데이터 범위**: 601회차 ~ 1205회차 (총 605회차)
...
**v6.1.1 (2026-01-24):** 🚑 배포 긴급 수정

- 🐛 **Streamlit Cloud 배포 오류 수정**: `altair<5` 버전 고정으로 `ModuleNotFoundError` 해결 (Python 3.13 환경 호환성 문제 대응)

**v5.1.1 (2026-01-24):** ⭐ 알고리즘 고도화 및 안정형 전략 추가

- 🛡️ **안정형(Safe) 전략 추가**: 원금 보존을 목표로 하는 5등 당첨 확률 극대화 전략 (필수 그룹 기반 조합)
- ⚖️ **상극수/궁합수 로직 개선**: 상극수 페널티 완화(-10점) 및 궁합수 보너스 강화(+0.5점)로 점수 산정 정교화
- 🎯 **결정론적 추천(Best Only) 모드**: 고정 모드 백테스팅 시 랜덤 요소를 제거하고 최적 조합 1개만 추천하는 옵션 추가
- 📊 **패턴 분석 강화**: 연속 번호, 궁합수/상극수, 소수/합성수 분석 로직 추가 (`pattern_analysis.py`)
- 🖥️ **UI 개선**: 웹 앱 내 전략 선택 및 백테스팅 옵션 업데이트
- 🐛 **버그 수정**: 백테스팅 시스템의 보너스 번호 참조 오류 수정
...
- ✅ 데이터 업데이트 시 앱 재시작 불필요 (mtime 기반 캐시 무효화)
- 📅 날짜 표시 개선: 시간 부분 제거 (2014.06.07 ~ 2026.01.03)
- 📊 **1205회차 데이터 추가** (2026.01.03)
  - 당첨번호: [1, 4, 16, 23, 31, 41]
  - 보너스: 2
  - 총 605회차로 업데이트
- 🔒 고정 모드 도움말 텍스트 동적 변경 (다음 회차 자동 계산)
- 🧪 검증 스크립트 추가
  - `verify_cache_fix.py`: 캐싱 수정 검증
  - `verify_update.py`: 데이터 업데이트 검증
- 🌐 Streamlit Cloud 배포: https://lo645251227.streamlit.app/
- 📝 README.md 및 CLAUDE.md 문서 업데이트

**기술적 세부사항:**

- `load_lotto_data()`: `@st.cache_data(ttl=60)` + `_file_mtime` 파라미터
- `load_prediction_model()`: 파일 수정 시간 기반 캐시 갱신
- `load_recommender()`: 파일 수정 시간 기반 캐시 갱신
- `load_core_system()`: 파일 수정 시간 기반 캐시 갱신
- `get_csv_file_mtime()`: CSV 파일 수정 시간 반환 헬퍼 함수
