...
| 9   | 🔬 **백테스팅 결과** ⭐   | 수익률 검증, 가중치 최적화, 실시간 재학습 |
| 10  | 🔄 **데이터 업데이트** ⭐ | 자동 크롤링, 텍스트 파싱, 수동 입력       |
...
2. **🛡️ 안정형 (Safe)** - 원금 보존 추구 (5등 당첨 확률 극대화) ⭐ NEW
3. **📊 점수 기반** - 번호별 종합 점수 (결정론적 추천 지원)
4. **🎲 확률 가중치** - 통계적 확률 샘플링
5. **🔄 패턴 기반** - 빈출 패턴 활용
6. **🎨 그리드 패턴** - 7x7 그리드 공간 분석
7. **🖼️ 이미지 패턴** - 시각적 밀도/균형
8. **🎰 무작위** - 순수 랜덤 (대조군)
